...
package org.apache.spark.sql.streaming.connector.s3
...
// This file is copied from Spark 3.2.1 with minor adaption to S3 connector
...
object RocksDBConf {
  ...
  val ROCKSDB_CONF_NAME_PREFIX = "spark.sql.streaming.s3connector.stateStore.rocksdb"
  ...
  def apply(sqlConf: SQLConf): RocksDBConf = {
    val confs = CaseInsensitiveMap[String](
      sqlConf.getAllConfs.filter(_._1.startsWith(ROCKSDB_CONF_NAME_PREFIX))
    )
  ...
  def apply(): RocksDBConf = apply(new SQLConf())
}
...
case class RocksDBNativeHistogram(
                                   sum: Long, avg: Double, stddev: Double,
                                   median: Double, p95: Double, p99: Double, count: Long) {
  def json: String = Serialization.write(this)(RocksDBMetrics.format)
}
